...
def main(df):  #主函数，取值并计算
    ...
    for loop in range(0,len(df),10):
        a = df['B4243_ST2_PLC_mdbotwidth_2'].iloc[loop]
        a_list.append(a)
        b = df['B4243_ST2_WATER_slabthick_2'].loc[loop]
        b_list.append(b)
        vc = df['B4243_ST2_WATER_castspeed_2'].loc[loop]
        vc_list.append(vc)
        t = df['B4243_GEN_PLC_tdsteelweight'].loc[loop]
        t_list.append(t)
        x = df['B4243_ST2_StopperPosition_2'].loc[loop]
        x_list.append(x)
    selected_data = {
        'a_value':a_list,
        'b_value':b_list,
    ...
    ans = judge(t_alarm=600, e_min=0.42, e_max=0.5, listx=selected_df['upper_e_value'], start_time=160)
    print(single_file + ans)
    return selected_df

if __name__ == '__main__':
    global file_dir
    global single_file
    file_dir = r'C:\Users\User\Desktop\jieliu_model\结瘤模型\6流'
    all_file_list = os.listdir(file_dir)
    for single_file in all_file_list:
        single_data_frame = pd.read_excel(os.path.join(file_dir,single_file),header=0)
        res_df = main(df=single_data_frame)
